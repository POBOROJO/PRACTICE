...
const GITA = path.join(__dirname, "../data/gita.pdf");
const PURANA = path.join(__dirname, "../data/puran.pdf");
...
const setupRAG = async () => {
  ...
  const gitaLoader = new PDFLoader(GITA);
  const puranaLoader = new PDFLoader(PURANA);

  const [gitaDocs, puranaDocs] = await Promise.all([
    gitaLoader.load(),
    puranaLoader.load(),
  ]);

  // Combine documents from both sources with source metadata
  const gitaDocsWithSource = gitaDocs.map((doc) => ({
    ...doc,
    metadata: { ...doc.metadata, source: "GITA" },
  }));

  const puranaDocsWithSource = puranaDocs.map((doc) => ({
    ...doc,
    metadata: { ...doc.metadata, source: "PURANA" },
  }));

  // Combine all documents
  const allDocs = [...gitaDocsWithSource, ...puranaDocsWithSource];
  console.log(
    `Loaded ${gitaDocs.length} pages from GITA and ${puranaDocs.length} pages from PURANA`
  );
  ...
  try {
    ...
    vectorStore = await FaissStore.load("./gita-vector-store", embeddings);
    console.log("Vector store loaded successfully");
  } catch (error) {
    // Create new vector store if none exists
    console.log("Creating new vector store...");
    vectorStore = await FaissStore.fromDocuments(splitDocs, embeddings);
    await vectorStore.save("./gita-vector-store");
...
const startInteractiveSession = async () => {
  ...
  console.log("\n🤖 GITA RAG System Ready!");
  console.log("Ask questions about GITA or PURANA, or type 'bye' to quit.\n");
